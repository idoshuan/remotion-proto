...
export function enhanceUserPrompt(topic: string): string {
  return [
    `Create an educational math video about: ${topic}.`,
    ...
    `The video should: 1) Start with an engaging title introducing the topic. 2) Visually explain the core concept with animated diagrams. 3) Show a concrete example with step-by-step solution. 4) End with a brief summary of the key takeaway.`,
